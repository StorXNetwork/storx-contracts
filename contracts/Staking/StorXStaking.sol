...
contract StroxStaking is Ownable {
    ...
    function canWithdrawStake() {
        ...
        require(coolOff < unstakeTenure, 'StorX: cool-off period not reached');
        _;
    }
    ...
    function claimEarned() public whenStaked canRedeemDrip {
        ...
        require(earnings < token.balanceOf(), 'StorX: insufficient contract balance');
    ...
    function canWithdrawStakeIn() public view returns (bool) {
        require(stakes[msg.sender].exists, 'StorX: stakeholder does not exists');
        require(stakes[msg.sender].staked == false, 'StorX: stakeholder still has stake');
        uint256 unstakeTenure = block.timestamp - stakes[msg.sender].unstakedTime;
    ...
    function setMaxStakeAmount(uint256 maxStakeAmount_) public onlyOwner {
        require(minStakeAmount_ > 0, 'StorX: maximum stake amount should be greater than 0');
    ...
    function withdrawXdc(address payable beneficiary_, uint256 amount_) public onlyOwner {
